...
def _run_subprocess(cmd: list[str], timeout: int) -> Optional[subprocess.CompletedProcess]:
    try:
        return subprocess.run(cmd, capture_output=True, text=True, timeout=timeout, creationflags=CREATION_FLAGS)
...
def parse_wmi_date(wmi_date_str: Any) -> Optional[str]:
    if not wmi_date_str:
        return None
    if not isinstance(wmi_date_str, str):
        return None
    if wmi_date_str.startswith("/Date("):
        try:
            ...
            return None
    if len(wmi_date_str) >= 19:
        return wmi_date_str[:19]
    if len(wmi_date_str) >= 10:
...
def _parse_os_batched(data: Optional[dict], hostname: str, os_info: dict) -> dict:
    base = {"system": os_info["system"], "release": os_info["release"], "version": os_info["version"], "cpu_arch": os_info["architecture"], "hostname": hostname}
    if not data:
        return base
    return {**base, "os_arch": data.get("OSArchitecture", "N/A"), "caption": data.get("Caption", "N/A"), "build_number": data.get("BuildNumber", "N/A"), "install_date": parse_wmi_date(data.get("InstallDate")), "last_bootup": parse_wmi_date(data.get("LastBootUpTime"))}


def _parse_cpu_batched(data: Optional[dict], processor: str) -> dict:
    if not data:
        return {"processor": processor}
    return {"processor": processor, "name": (data.get("Name") or "N/A").strip(), "cores": safe_int(data.get("NumberOfCores")), "logical_processors": safe_int(data.get("NumberOfLogicalProcessors")), "max_clock_speed_mhz": data.get("MaxClockSpeed", "N/A"), "l2_cache_kb": data.get("L2CacheSize", "N/A"), "l3_cache_kb": data.get("L3CacheSize", "N/A")}
...
def _parse_memory_batched(data: Optional[Any]) -> dict:
    ...
    for stick in sticks_list:
        ...
        gb = _bytes_to_gb(cap)
        total_gb += gb
        sticks.append({"capacity_gb": gb, "speed_mhz": stick.get("Speed", "N/A"), "manufacturer": stick.get("Manufacturer", "N/A"), "part_number": (stick.get("PartNumber") or "N/A").strip()})
...
def _parse_disk_batched(data: Optional[Any]) -> dict:
    ...
    for drive in drives_list:
        ...
        drives.append({"drive": drive.get("DeviceID", "N/A"), "type": "本地磁盘", "file_system": drive.get("FileSystem", "N/A"), "total_gb": _bytes_to_gb(size_bytes) if size_bytes > 0 else 0, "free_gb": _bytes_to_gb(free_bytes) if free_bytes > 0 else 0, "volume_name": drive.get("VolumeName", "")})
...
def _parse_gpu_batched(data: Optional[Any]) -> dict:
    ...
    for gpu in gpus_list:
        ...
        gpus.append({"name": (gpu.get("Name") or "N/A").strip(), "vram_gb": _bytes_to_gb(vram), "driver_version": gpu.get("DriverVersion", "N/A"), "video_processor": gpu.get("VideoProcessor", "N/A"), "video_mode": gpu.get("VideoModeDescription", "N/A")})
    return {"gpus": gpus}
...
def _parse_motherboard_batched(data: Optional[dict]) -> dict:
    if not data:
        return {}
    return {"manufacturer": data.get("Manufacturer", "N/A"), "product": data.get("Product", "N/A"), "serial_number": data.get("SerialNumber", "N/A"), "version": data.get("Version", "N/A")}


def _parse_bios_batched(data: Optional[dict]) -> dict:
    if not data:
        return {}
    return {"manufacturer": data.get("Manufacturer", "N/A"), "name": data.get("Name", "N/A"), "version": data.get("Version", "N/A"), "release_date": parse_wmi_date(data.get("ReleaseDate")), "serial_number": data.get("SerialNumber", "N/A")}
...
def _parse_network_batched(data: Optional[Any]) -> dict:
    ...
    for nic in nics_list:
        adapters.append({"name": nic.get("Name", "N/A"), "mac_address": nic.get("MACAddress", "N/A"), "speed": format_speed(safe_int(nic.get("Speed"))), "adapter_type": nic.get("AdapterType", "N/A")})
...
_script_hash_cache: dict[str, tuple[str, float]] = {}
...
def collect_all_batched(hostname: str, processor: str, os_info: dict) -> Optional[dict]:
    ...
    if not ps_path.exists():
        ...
        return None
    ps_mtime = ps_path.stat().st_mtime
    cache_key = str(ps_path)
    hash_path = ps_path.with_suffix(".ps1.sha256")
    cached_entry = _script_hash_cache.get(cache_key)
    if cached_entry and cached_entry[0] == ps_mtime:
        current_hash = cached_entry[1]
    else:
        current_hash = hashlib.sha256(ps_path.read_bytes()).hexdigest()
        _script_hash_cache[cache_key] = (ps_mtime, current_hash)
    if hash_path.exists():
    ...
    result = _run_subprocess(["powershell", "-NoProfile", "-ExecutionPolicy", "RemoteSigned", "-File", str(ps_path)], TIMEOUT_BATCH)
    if result is None:
    ...
    info = {"采集时间": _now_str(), "计算机名": hostname, "操作系统": _parse_os_batched(d.get("os"), hostname, os_info), "CPU": _parse_cpu_batched(d.get("cpu"), processor), "内存": _parse_memory_batched(d.get("memory")), "磁盘": _parse_disk_batched(d.get("disk")), "显卡": _parse_gpu_batched(d.get("gpu")), "主板": _parse_motherboard_batched(d.get("motherboard")), "BIOS": _parse_bios_batched(d.get("bios")), "网络适配器": _parse_network_batched(d.get("network"))}
    info["快速摘要"] = {"hostname": hostname, "os": platform.system(), "os_version": s.get("os_version", platform.version()), "cpu": (s.get("cpuName") or "N/A").strip(), "cpu_cores": s.get("cpuCores", "N/A"), "cpu_threads": s.get("cpuThreads", "N/A"), "ram_gb": s.get("ramGB", "N/A"), "disk_total_gb": s.get("diskTotalGB", 0), "disk_free_gb": s.get("diskFreeGB", 0), "gpu": " / ".join(s.get("gpuNames", [])) or "N/A"}
    return info
...
def collect_all() -> Optional[dict]:
    ...
    os_info = {"system": platform.system(), "release": platform.release(), "version": platform.version(), "architecture": platform.machine()}
    ...
    for label in ("操作系统", "CPU", "内存", "磁盘", "显卡", "主板", "BIOS", "网络适配器"):
        info[label] = {}
    for i, (label, collector_fn, *args) in enumerate([("操作系统", _collect_os, os_info, hostname), ("CPU", _collect_cpu, processor), ("内存", _collect_memory,), ("磁盘", _collect_disk,), ("显卡", _collect_gpu,), ("主板", _collect_motherboard,), ("BIOS", _collect_bios,), ("网络适配器", _collect_network,)], 1):
...
def _collect_os(os_info: dict, hostname: str) -> dict:
    ...
    if data:
        return _parse_os_batched(data, hostname, os_info)
    return {"system": os_info["system"], "release": os_info["release"], "version": os_info["version"], "cpu_arch": os_info["architecture"], "hostname": hostname}
...
def _collect_memory() -> dict:
    ...
    stick_list = []
    for s in sticks:
        cap = safe_int(s.get("Capacity"), 0) or 0
        stick_list.append({"capacity_gb": _bytes_to_gb(cap), "speed_mhz": s.get("Speed", "N/A"), "manufacturer": s.get("Manufacturer", "N/A"), "part_number": s.get("PartNumber", "N/A")})
    return {"total_gb": total_gb, "sticks": stick_list}
...
def _collect_disk() -> dict:
    ...
    for d in rows:
        dt = d.get("DriveType", "")
        size_b = safe_int(d.get("Size"), 0) or 0
        free_b = safe_int(d.get("FreeSpace"), 0) or 0
        if dt == "3":
            total_size += size_b
            total_free += free_b
...
def _collect_gpu() -> dict:
    ...
    gpus = []
    for g in rows:
        gpus.append({"name": (g.get("Name", "N/A") or "").strip(), "vram_gb": _bytes_to_gb(safe_int(g.get("AdapterRAM"), 0) or 0), "driver_version": g.get("DriverVersion", "N/A"), "video_processor": g.get("VideoProcessor", "N/A")})
    return {"gpus": gpus}
...
def _collect_network() -> dict:
    ...
    adapters = []
    for n in rows:
        adapters.append({"name": n.get("Name", "N/A"), "mac_address": n.get("MACAddress", "N/A"), "speed": format_speed(safe_int(n.get("Speed"))), "adapter_type": n.get("AdapterType", "N/A")})
    return {"adapters": adapters}
...
def _build_summary_from_info(info: dict, hostname: str, os_info: dict) -> dict:
    ...
    return {"hostname": hostname, "os": os_info["system"], "os_version": os_info["version"], "cpu": cpu_info.get("name", cpu_info.get("processor", "N/A")), "cpu_cores": cpu_info.get("cores", "N/A"), "cpu_threads": cpu_info.get("logical_processors", "N/A"), "ram_gb": mem_info.get("total_gb", "N/A"), "disk_total_gb": disk_info.get("total_size_gb", 0), "disk_free_gb": disk_info.get("total_free_gb", 0), "gpu": " / ".join(gpu_names) or "N/A"}


def _acquire_file_lock(file_obj, retries: int = 3, delay_ms: int = 50) -> bool:
    if msvcrt is None:
        return True
    import time
    ...
    try:
        ...
        with open(file_path, "wb") as f:
            ...
            while remaining > 0:
                ...
                f.write(SECURE_DELETE_OVERWRITE_BYTE * write_size)
...
def _export_build_lines(info: dict) -> list[str]:
    ...
    gpu_info = info.get("显卡", {})
    mb_info = info.get("主板", {})
    lines = ["=" * 60, "              电脑配置信息报告", "=" * 60, f"采集时间: {info.get('采集时间', 'N/A')}", f"计算机名: {info.get('计算机名', 'N/A')}", "", "【操作系统】", f"  系统名称: {os_info.get('caption', os_info.get('system', 'N/A'))}", f"  版本号:   {os_info.get('version', 'N/A')}", f"  架构:     {os_info.get('os_arch', os_info.get('cpu_arch', 'N/A'))}", f"  Build号:  {os_info.get('build_number', 'N/A')}", f"  安装日期: {os_info.get('install_date', 'N/A')}", f"  启动时间: {os_info.get('last_bootup', 'N/A')}", "", "【处理器】", f"  型号:     {cpu_info.get('name', cpu_info.get('processor', 'N/A'))}", f"  核心数:   {cpu_info.get('cores', 'N/A')}", f"  线程数:   {cpu_info.get('logical_processors', 'N/A')}", f"  主频:     {cpu_info.get('max_clock_speed_mhz', 'N/A')} MHz", f"  L2缓存:   {cpu_info.get('l2_cache_kb', 'N/A')} KB", f"  L3缓存:   {cpu_info.get('l3_cache_kb', 'N/A')} KB", "", "【内存】", f"  总容量:   {mem_info.get('total_gb', 'N/A')} GB"]
    ...
    lines.extend(["", "【磁盘】", f"  总容量:   {disk_info.get('total_size_gb', 'N/A')} GB", f"  可用:     {disk_info.get('total_free_gb', 'N/A')} GB"])
    ...
    for gpu in gpu_info.get("gpus", []):
        lines.extend([f"  型号:     {gpu.get('name', 'N/A')}", f"  显存:     {gpu.get('vram_gb', 'N/A')} GB", f"  驱动:     {gpu.get('driver_version', 'N/A')}"])
    if mb_info:
        lines.extend(["", "【主板】", f"  制造商:   {mb_info.get('manufacturer', 'N/A')}", f"  型号:     {mb_info.get('product', 'N/A')}", f"  版本:     {mb_info.get('version', 'N/A')}"])
    lines.extend(["", "=" * 60])
    return lines
...
def export_csv(info: dict, output_path: Optional[str] = None) -> str:
    ...
    gpu_info = info.get("显卡", {})
    mb_info = info.get("主板", {})
    rows = [["配置项", "值"], ["采集时间", info.get("采集时间", "N/A")], ["计算机名", info.get("计算机名", "N/A")], ["系统名称", os_info.get("caption", os_info.get("system", "N/A"))], ["系统版本", os_info.get("version", "N/A")], ["系统架构", os_info.get("os_arch", os_info.get("cpu_arch", "N/A"))], ["Build号", os_info.get("build_number", "N/A")], ["安装日期", os_info.get("install_date", "N/A")], ["启动时间", os_info.get("last_bootup", "N/A")], ["CPU型号", cpu_info.get("name", cpu_info.get("processor", "N/A"))], ["CPU核心数", cpu_info.get("cores", "N/A")], ["CPU线程数", cpu_info.get("logical_processors", "N/A")], ["CPU主频(MHz)", cpu_info.get("max_clock_speed_mhz", "N/A")], ["CPU L2缓存(KB)", cpu_info.get("l2_cache_kb", "N/A")], ["CPU L3缓存(KB)", cpu_info.get("l3_cache_kb", "N/A")], ["内存总容量(GB)", mem_info.get("total_gb", "N/A")]]
    for i, stick in enumerate(mem_info.get("sticks", []), 1):
        rows.extend([[f"内存条{i}_容量(GB)", stick.get("capacity_gb", "N/A")], [f"内存条{i}_频率(MHz)", stick.get("speed_mhz", "N/A")], [f"内存条{i}_品牌", stick.get("manufacturer", "N/A")], [f"内存条{i}_型号", stick.get("part_number", "N/A")]])
    rows.extend([["磁盘总容量(GB)", disk_info.get("total_size_gb", "N/A")], ["磁盘可用(GB)", disk_info.get("total_free_gb", "N/A")]])
    for d in disk_info.get("drives", []):
        rows.extend([[f"磁盘_{d.get('drive', 'N/A')}_总容量(GB)", d.get("total_gb", "N/A")], [f"磁盘_{d.get('drive', 'N/A')}_可用(GB)", d.get("free_gb", "N/A")], [f"磁盘_{d.get('drive', 'N/A')}_文件系统", d.get("file_system", "N/A")]])
    for i, gpu in enumerate(gpu_info.get("gpus", []), 1):
        rows.extend([[f"显卡{i}_型号", gpu.get("name", "N/A")], [f"显卡{i}_显存(GB)", gpu.get("vram_gb", "N/A")], [f"显卡{i}_驱动", gpu.get("driver_version", "N/A")]])
    if mb_info:
        rows.extend([["主板制造商", mb_info.get("manufacturer", "N/A")], ["主板型号", mb_info.get("product", "N/A")]])
...
def compare_configs(config1: dict, config2: dict, max_depth: int = 20) -> list[dict]:
    differences = []

    def _cmp(a: Any, b: Any, path: str = "root", depth: int = 0, seen_pairs: set = None):
        ...
        if isinstance(a, dict):
            ...
            for k in keys:
                ...
                if k not in a:
                    ...
                    _cmp(a[k], b[k], np, depth + 1, seen_pairs)
        elif isinstance(a, list):
            la, lb = len(a), len(b)
            if la != lb:
                differences.append({"path": path, "old": f"长度 {la}", "new": f"长度 {lb}"})
            for i in range(min(la, lb)):
                _cmp(a[i], b[i], f"{path}[{i}]", depth + 1, seen_pairs)
...
def format_comparison(differences: list, t1: str, t2: str) -> str:
    ...
    for title, items in sections:
        if items:
            ...
            for d in items:
                lines.extend([f"  {d['path']}", f"    旧: {d['old_value']}", f"    新: {d['new_value']}"])
    return "\n".join(lines)


def delete_all_data() -> int:
    deleted = 0
    for pat in ("config.json", "config_*.txt", "config_*.csv", "snapshot_*.json", "config*.tmp", "collect_all.ps1.sha256"):
...
def import_config(import_path: str) -> bool:
    ...
    if resolved.suffix != ".json":
        ...
        return False
    allowed_dirs = [CACHE_DIR.resolve(), Path.cwd().resolve(), Path.home().resolve()]
    import tempfile
    allowed_dirs.append(Path(tempfile.gettempdir()).resolve())
    if not any(resolved.is_relative_to(d) for d in allowed_dirs):
        print(f"错误: 导入文件路径不合法，仅允许从缓存目录、当前目录、用户主目录或临时目录导入")
        return False
    try:
    ...
    for f in sorted(SNAPSHOTS_DIR.glob("snapshot_*.json"), reverse=True):
        try:
            with open(f, "r", encoding="utf-8") as fh:
                d = json.load(fh)
            snapshots.append({"file": f.name, "path": str(f), "time": d.get("快照时间", d.get("采集时间", "未知")), "label": d.get("快照标签", ""), "hostname": d.get("计算机名", "N/A")})
...
def assess_system_health(data: dict) -> dict:
    ...
    if total_disk is not None:
        if total_disk == 0:
            health["details"]["磁盘使用率"] = "异常 (0 GB)"
            health["score"] -= 20
            health["warnings"].append("磁盘总容量为 0，可能存在配置异常")
        elif total_disk > 0:
        ...
        for drive in disk_info.get("drives", []):
            ...
            if dt and dt > 0 and ((dt - df) / dt) * 100 > DISK_DRIVE_NEARLY_FULL:
                health["warnings"].append(f"驱动器 {drive.get('drive', '?')} 几乎已满 ({((dt-df)/dt)*100:.1f}%)，仅剩 {df:.1f} GB")
...
def format_health_report(health: dict) -> str:
    ...
    if score >= HEALTH_SCORE_EXCELLENT:
        ...
        grade, indicator = "需关注", "🔴"
    lines = ["═══════════════════════════════════════", "          系统健康评估报告", "═══════════════════════════════════════", f"  综合评分: {score}/100 {indicator} {grade}", "───────────────────────────────────────"]
    if health["details"]:
        lines.append("  【系统指标】")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.cache_path:
            print(CACHE_FILE)
        need_config = not (args.check_cache or args.cache_path or args.delete_data or args.list_snapshots or args.import_config or args.compare)
        if not need_config:
            sys.exit(0)
